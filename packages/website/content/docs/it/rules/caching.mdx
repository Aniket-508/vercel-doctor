...
- **Next.js 15**: gli avvisi sottolineano che `fetch` e gli handler GET non sono memorizzati nella cache per impostazione predefinita.
- **Next.js 16+**: gli avvisi danno priorità alla guida su Cache Components (`"use cache"`, cache tag, revalidation mirata).
- **Altre/versioni sconosciute**: gli avvisi usano una guida sulle policy di cache indipendente dal framework.
