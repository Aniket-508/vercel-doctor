...
- **Next.js 15**: os avisos enfatizam que `fetch` e handlers GET não são armazenados em cache por predefinição.
- **Next.js 16+**: os avisos priorizam a orientação sobre Cache Components (`"use cache"`, tags de cache, revalidação direcionada).
- **Outras/versões desconhecidas**: os avisos usam orientação agnóstica ao framework sobre políticas de cache.
