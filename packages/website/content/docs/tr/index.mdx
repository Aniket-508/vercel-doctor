...
Vercel Doctor, Vercel üzerinde dağıtılan Next.js projeleri için özel bir sağlık kontrolü aracıdır. Faturanızı şişiren kalıpları tarar — fonksiyon süresi, önbelleğe alınmamış rotalar, optimize edilmemiş görseller ve daha fazlası — ardından uygulanabilir düzeltmelerle 100 üzerinden bir puan verir.
...
| ------------------------------------------------------ | -------- | ----------------------------------------------------------------------------------------------------- |
| [Fonksiyon Süresi](/docs/rules/function-duration)      | 3        | Sıralı await'ler, engelleyici loglama, paralel olmayan I/O                                            |
| [Önbellek](/docs/rules/caching)                        | 6        | `force-dynamic`, eksik önbellek politikaları, `no-store` fetch'ler, GET handler'lardaki yan etkiler   |
| [Çağrılar](/docs/rules/invocations)                    | 2        | Sayfalarda istemci taraflı fetch, agresif link prefetch'i                                             |
| [Görsel Optimizasyonu](/docs/rules/image-optimization) | 4        | Global `unoptimized`, eksik `sizes`, geniş uzak kalıplar, `unoptimized` olmadan SVG                   |
| [Platform](/docs/rules/platform)                       | 8        | Edge'de ağır import'lar, cron job'lar, Bun runtime, Fluid Compute, deploy arşivi, Turbopack önbelleği |
| [Ölü Kod](/docs/rules/dead-code)                       | 4        | Kullanılmayan dosyalar, export'lar, tipler ve tekrarlar                                               |
